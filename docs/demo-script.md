...
3. Add or edit goals until total weightage is exactly `100%`.
4. Submit the goal sheet for manager approval.
5. After approval, return as Employee and try editing goals to show the lock behavior.
6. If a quarter window is active, enter actual achievement and status.
...
3. Adjust a quarter window if the demo needs progress capture now.
4. Create a shared goal and push it to employees.
5. Unlock a locked goal sheet with a reason.
6. Export the achievement CSV and show audit logs.
